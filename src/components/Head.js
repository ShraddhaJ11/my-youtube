...
const Head = () => {
  ...
  const [suggestations, setSuggesatation] = useState([]);
  const [showSuggestation, setShowSuggestation] = useState(false);

  const searchCache = useSelector((store) => store.search);
  const dispatch = useDispatch();
  /**
   * searchcache = {
   *    "iphone":["ipone 11","iphone 12"]
   * }
   */
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (searchCache[searchQuery]) {
        setSuggesatation(searchCache[searchQuery]);
      } else {
   ...
   *
   * key - i
   *   render the component and call useEffect
   *   start timer -> make api call after 200ms
   *
   * key - ip
   *   destroy the component(useEffect return method also)
   *   re-render the component and call useEffect
   *   start new timer -> make api call after 200ms
   *
   * setTimeout(200) make an api call
   */

  const getSearchSuggestations = async () => {
    console.log("API call - " + searchQuery);
    const data = await fetch(YOUTUBE_SEARCH_API + searchQuery);
    const json = await data.json();
    setSuggesatation(json[1]);
    dispatch(
      cacheResults({
        [searchQuery]: json[1],
      })
      ...
      <div className="col-span-10 px-10">
        <div>
          <input
            ...
            onBlur={() => setShowSuggestation(false)}
            onFocus={() => setShowSuggestation(true)}
          />
